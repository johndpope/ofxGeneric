...
void ofxGenericSoundManager::init( ofPtr< ofxGenericSoundManager > setThis )
{
    if ( _this )
    {
        // TZLA-619 // ofLogWarning( "ofxGenericSoundManager already has _this set and init is being called! Are you calling ofxGenericSoundManager::getInstance() before calling you subclass's getInstance()?" );
    }
...
bool ofxGenericSoundManager::soundExistsCheck( string sound )
{
    if ( !_soundMap[ sound ] )
    {
        // TZLA-619 // ofLogWarning( "ofxGenericSoundManager: There is no sound loaded named \"" + sound + "\"" );
        return false;
    }
    return true;
}
